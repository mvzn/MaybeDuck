...
inline double peak_gain_for_q(double Q) {if (Q <= 0.707) return 1.0; const double denom = Q * Q - 0.25; if (denom <= 0.0) return (Q*Q) / (sqrt(denom));}
...
protected:
    ...
    {
        if (m_detect_params.attack_ms == attack_in_ms && forceCalc == false)
            return;
    ...
    {
        if (m_detect_params.release_ms == release_in_ms && forceCalc == false)
            return;
        m_detect_params.release_ms = std::max(release_in_ms, 0.001);
        release_time = exp(TLD_AUDIO_ENVELOPE_ANALOG_TC / (release_in_ms * fs * 0.001));
    }
};
